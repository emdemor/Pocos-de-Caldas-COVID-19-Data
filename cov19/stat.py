...
class stat_model:
    ...
    def chi_sqrd(self,
        ...
        # chi2 function
        chi2 = lambda model, data: (model-data)**2/model
        ...
        if fit_confirmed:
            list_c = np.array(list(map(chi2, list(np.interp(t_data, t_model, c_model)), c_data)))
            list_c = list_c[~np.isnan(list_c)]
            Chi2 += weight_confirmed*list_c.sum()#sum(list_c)
            # print(Chi2)
            

        # summing recovered part
        if fit_recovered:
            list_r = np.array(list(map(chi2, list(np.interp(t_data, t_model, c_model)), c_data)))
            list_r = list_r[~np.isnan(list_r)]
            Chi2 += weight_recovered*list_r.sum()#sum(list_r)
            # print('recovered chi2',Chi2)
            # print('w',weight_recovered)
            # print('l',list(map(chi2, list(np.interp(t_data, t_model, r_model)), r_data)))
            # print('\n\n')
            

        # summing death part
        if fit_death:
            list_d = np.array(list(map(chi2, list(np.interp(t_data, t_model, c_model)), c_data)))
            list_d = list_d[~np.isnan(list_d)]
            Chi2 += weight_death*list_d.sum()#sum(list_d)
            # print('death chi2',Chi2)
            # print('w',weight_death)
            # print('l',list(map(chi2, list(np.interp(t_data, t_model, d_model)), d_data)))
            # print('\n\n')
